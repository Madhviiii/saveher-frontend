...
import { sendDirectEmergencySMS, testDirectSMS } from '@/lib/directSMS';
import { sendFreeEmergencyAlert, openWhatsAppForContact } from '@/lib/freeMessaging';
...
export const SafetyProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const notifyGuardiansWithLocation = async (type: 'danger_zone' | 'sos' | 'doubt', location: { lat: number; lng: number }, zone?: DangerZone) => {
    ...
    try {
      ...
      // Send Direct SMS
      const smsResults = await sendDirectEmergencySMS(
        contacts,
        type,
        location,
        address,
      ...
      toast.error('Direct SMS failed, using WhatsApp backup');
    }

    // WhatsApp Backup
    try {
      console.log('📱 Sending WhatsApp backup...');
      const whatsappResults = await sendFreeEmergencyAlert(
        contacts,
        type,
        location,
        address,
      ...
      console.log('📱 WhatsApp Backup Results:');
      console.log(`WhatsApp: ${whatsappResults.whatsapp ? '✅' : '❌'}`);
      console.log(`Email: ${whatsappResults.email ? '✅' : '❌'}`);

      // Auto-open WhatsApp for first contact
      if (whatsappResults.whatsapp && contacts.length > 0) {
        const firstContact = contacts[0];
        const message = type === 'sos' 
          ? `🆘 EMERGENCY SOS ALERT! User triggered emergency SOS. Time: ${new Date().toLocaleString()}. 📍 Address: ${address}. 🗺️ Maps: https://maps.google.com/?q=${location.lat},${location.lng}. 📞 CALL IMMEDIATELY!`
          : type === 'danger_zone'
          ? `🚨 DANGER ZONE ALERT! User entered: ${zone?.name}. Time: ${new Date().toLocaleString()}. 📍 Address: ${address}. 🗺️ Maps: https://maps.google.com/?q=${location.lat},${location.lng}`
          : `🛡️ DOUBT MODE ALERT! User feels unsafe. Time: ${new Date().toLocaleString()}. 📍 Address: ${address}. 🗺️ Maps: https://maps.google.com/?q=${location.lat},${location.lng}`;
        
        setTimeout(() => {
          openWhatsAppForContact(firstContact, message);
        }, 1000);
      }
    ...
    // Final Summary
    console.log('📊 COMPLETE EMERGENCY SUMMARY:');
    console.log(`📱 Real SMS: ${directSMSSuccessful} sent, ${directSMSFailed} failed`);
    console.log(`📱 WhatsApp: ${contacts.length} contacts ready`);
    console.log(`🎯 Total contacts reached: ${directSMSSuccessful + contacts.length}`);
  };

  const triggerDoubtMode = () => {
    createAlert('doubt');
    toast.warning('Doubt Mode Activated', {
      description: 'Silently sharing your location with emergency contacts',
      duration: 5000,
    });
  };
  ...
  const startAudioDetection = () => {
    setIsListening(true);
    toast.info('Audio Detection Active', {
      description: 'Listening for distress sounds...',
      duration: 5000,
    });
  };
  ...
  const testDirectSMSMessaging = async () => {
    ...
    try {
      ...
      const result = await testDirectSMS(testContact.phone);
